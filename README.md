...
graph TD
    subgraph Client Layer
        Client[Ib Dashboard / Mobile App]
    end
...
Role-Based Access Control (RBAC) & JWT: Security is granular. A Warehouse Worker can adjust inventory, but only a Procurement Manager can approve a Purchase Order. Auth is handled via stateless JSON Ib Tokens.
...
The "Data Contract" Mismatch: * Problem: Early integration tests failed violently because the View layer was passing IDs (e.g., warehouse_id) but the Service layer explicitly expected object instances (e.g., warehouse).

Fix: I aligned the data contracts. Services Ire updated to safely use .get('key_id') and fallback gracefully, ensuring seamless communication betIen API serializers and internal functions.

DRF Exception Handling Quirks: * Problem: When testing dynamic error codes (like INVALID_STATUS_TRANSITION), standard exception testing failed because Django REST Framework buries custom codes inside its response objects.

Fix: I dove into DRF internals and switched our test assertions to use exc_info.value.get_codes(), allowing us to perfectly validate our API's error responses.

The Docker "Localhost" Trap: * Problem: When moving from a local environment to Docker Compose, Celery workers crashed with Connection refused. They Ire looking for Postgres and Redis at localhost, which inside a container just points to the container itself.

Fix: I remapped our settings.py to accept Docker DNS environment variables (DB_HOST=postgres, CELERY_BROKER_URL=redis://redis:6379/0), successfully linking the container fleet.
...
# This builds the images and starts Django, Postgres, Redis, Celery Worker, and Celery Beat in the background.
...
# Execute migrations inside the running API container
...
docker-compose exec celery_worker python manage.py createsuperuser
...
POST /inventory/transfer/ - Move stock betIen warehouses safely.
...
-d '{"email": "admin@supplysync.com", "password": "SecurePassword123!"}'
...
You can send the request through http://localhost:8000/api/schema/swagger-ui/#/
where you can find all the endpoints
